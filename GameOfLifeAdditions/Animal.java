...
* @see Creature
*/
public class Animal extends Creature implements Movable, Aggressor, Spawner, Sleeper {
	...
	// Spawn a new 20% of the time after eating
	public Creature spawnNewCreature() {
		if (_rand.nextInt(4) == 0 && canSpawn) {
			Animal spawn = new Animal();
	...
	public void sleep() {
		if (isSleeping) {
			if (sleepTimer == 0) {
				isSleeping = false;
				sleepTimer = _rand.nextInt(100); // To stay awake
			}
			else {
				sleepTimer--;
			}
			...
			if (sleepTimer == 0) {
				isSleeping = true;
				sleepTimer = _rand.nextInt(100); // To sleep
			}
			else {
				sleepTimer--;
			}
